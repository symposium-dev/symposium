...
impl Agent {
    ...
    pub fn register_project_hooks(
        ...
        match self {
            ...
            Agent::OpenCode => {
                register_opencode_plugin(&project_root.join(".opencode").join("plugins"), out)
            }
        }?;
    ...
    pub fn register_hooks(&self, home: &Path, _sym: &Symposium, out: &Output) -> Result<()> {
        ...
        match self {
            ...
            Agent::OpenCode => register_opencode_plugin(
    ...
    pub fn unregister_project_hooks(&self, project_root: &Path, _sym: &Symposium, out: &Output) {
        match self {
            ...
            Agent::OpenCode => {
                unregister_opencode_plugin(&project_root.join(".opencode").join("plugins"), out)
            }
        }
    }
    ...
    pub fn unregister_hooks(&self, home: &Path, _sym: &Symposium, out: &Output) {
        match self {
            ...
            Agent::Goose => {} // no hooks to unregister
            Agent::OpenCode => unregister_opencode_plugin(
                &home.join(".config").join("opencode").join("plugins"),
                out,
            ),
        }
...
const OPENCODE_PLUGIN_SOURCE: &str = include_str!("../../opencode-plugin/src/index.ts");
const OPENCODE_PLUGIN_HEADER: &str = "// @generated by symposium — do not edit\n";

fn opencode_plugin_content() -> String {
    format!("{OPENCODE_PLUGIN_HEADER}{OPENCODE_PLUGIN_SOURCE}")
}

fn register_opencode_plugin(plugins_dir: &Path, out: &Output) -> Result<()> {
    fs::create_dir_all(plugins_dir)?;
    let plugin_file = plugins_dir.join("symposium.ts");
    let display = display_path(&plugin_file);
    let expected = opencode_plugin_content();

    if plugin_file.exists() {
        let existing = fs::read_to_string(&plugin_file)?;
        if existing == expected {
            out.already_ok(format!("{display}: plugin already up to date"));
            return Ok(());
        }
        if !existing.starts_with(OPENCODE_PLUGIN_HEADER) {
            out.warn(format!(
                "{display}: file exists but was not generated by symposium; skipping"
            ));
            return Ok(());
        }
    }

    fs::write(&plugin_file, &expected)?;
    out.done(format!("{display}: installed hook bridge plugin"));
    Ok(())
}

fn unregister_opencode_plugin(plugins_dir: &Path, out: &Output) {
    let plugin_file = plugins_dir.join("symposium.ts");
    if !plugin_file.exists() {
        return;
    }
    let display = display_path(&plugin_file);
    if let Ok(content) = fs::read_to_string(&plugin_file) {
        if !content.starts_with(OPENCODE_PLUGIN_HEADER) {
            return;
        }
    }
    if fs::remove_file(&plugin_file).is_ok() {
        out.removed(format!("{display}: removed hook bridge plugin"));
    }
}
...
mod tests {
    ...
    #[test]
    fn register_opencode_plugin_creates_file() {
        let tmp = tempfile::tempdir().unwrap();
        let plugins_dir = tmp.path().join("plugins");
        register_opencode_plugin(&plugins_dir, &Output::quiet()).unwrap();

        let plugin_file = plugins_dir.join("symposium.ts");
        assert!(plugin_file.exists());
        let content = fs::read_to_string(&plugin_file).unwrap();
        assert!(content.starts_with(OPENCODE_PLUGIN_HEADER));
    ...
    #[test]
    fn register_opencode_plugin_upgrades_stale() {
        let tmp = tempfile::tempdir().unwrap();
        let plugins_dir = tmp.path().join("plugins");
        ...
        fs::write(
            &plugin_file,
            format!("{OPENCODE_PLUGIN_HEADER}// old version"),
        )
        .unwrap();
        ...
        assert!(
            ...
            "should have been overwritten with current content"
        );
    }

    #[test]
    fn register_opencode_plugin_skips_user_managed() {
        let tmp = tempfile::tempdir().unwrap();
        let plugins_dir = tmp.path().join("plugins");
        fs::create_dir_all(&plugins_dir).unwrap();

        let plugin_file = plugins_dir.join("symposium.ts");
        let user_content = "// user's custom plugin\nexport const server = async () => ({});\n";
        fs::write(&plugin_file, user_content).unwrap();
        ...
        assert_eq!(
            ...
            "should not overwrite user-managed file"
        );
    }

    #[test]
    fn unregister_opencode_plugin_removes_file() {
        let tmp = tempfile::tempdir().unwrap();
        let plugins_dir = tmp.path().join("plugins");
        register_opencode_plugin(&plugins_dir, &Output::quiet()).unwrap();
        ...
        unregister_opencode_plugin(&plugins_dir, &Output::quiet());
        assert!(!plugin_file.exists());
    }

    #[test]
    fn unregister_opencode_plugin_preserves_user_managed() {
        let tmp = tempfile::tempdir().unwrap();
        let plugins_dir = tmp.path().join("plugins");
        fs::create_dir_all(&plugins_dir).unwrap();
        ...
        unregister_opencode_plugin(&plugins_dir, &Output::quiet());
        assert!(plugin_file.exists(), "should not remove user-managed file");
    }
