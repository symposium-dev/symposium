...
/// Predicate-gated recursive `source.crate` entries are followed when the
/// declaring plugin's source context and workspace predicates match.
#[tokio::test]
async fn sync_recursive_crate_source_gate_matches() {
    with_fixture(
        TestMode::SimulationOnly,
        &["recursive-crate-gated0", "workspace0"],
        async |mut ctx| {
            ctx.symposium(&["init", "--add-agent", "claude"]).await?;
            ctx.symposium(&["sync"]).await?;
...
/// Predicate-gated recursive `source.crate` entries are not followed when the
/// gate does not match the current workspace.
#[tokio::test]
async fn sync_recursive_crate_source_gate_blocks() {
    with_fixture(
        TestMode::SimulationOnly,
        &["recursive-crate-gated0", "workspace-noserde0"],
        async |mut ctx| {
            ctx.symposium(&["init", "--add-agent", "claude"]).await?;
            ctx.symposium(&["sync"]).await?;

            let workspace_root = ctx.workspace_root.as_ref().unwrap();
            let found = find_installed_skills(
                &workspace_root.join(".claude/skills"),
                "gated-recursive-skill",
            );
            assert!(
                found.is_empty(),
                "gated-recursive-skill should not install when serde is absent, found: {found:?}"
            );
            Ok(())
        },
    )
...
/// Predicate-gated recursive `source.path` entries are followed only when the
/// gate matches the current workspace.
#[tokio::test]
async fn sync_recursive_path_source_gate_matches_and_blocks() {
    with_fixture(
        TestMode::SimulationOnly,
        &["recursive-path-gated0", "workspace0"],
        async |mut ctx| {
            ctx.symposium(&["init", "--add-agent", "claude"]).await?;
            ctx.symposium(&["sync"]).await?;

            let workspace_root = ctx.workspace_root.as_ref().unwrap();
            find_installed_skill(&workspace_root.join(".claude/skills"), "gated-path-skill");
            Ok(())
        },
    )
    .await
    ...
    with_fixture(
        TestMode::SimulationOnly,
        &["recursive-path-gated0", "workspace-noserde0"],
        async |mut ctx| {
            ctx.symposium(&["init", "--add-agent", "claude"]).await?;
            ctx.symposium(&["sync"]).await?;
            ...
            let workspace_root = ctx.workspace_root.as_ref().unwrap();
            let found =
                find_installed_skills(&workspace_root.join(".claude/skills"), "gated-path-skill");
            assert!(
                found.is_empty(),
                "gated-path-skill should not install when serde is absent, found: {found:?}"
            );
            Ok(())
        },
    )
    ...
    with_fixture(
        ...
        async |mut ctx| {
            ...
            let plugin_events: Vec<_> = events
                ...
                .collect();
            let inactive = plugin_events.iter().find(|v| {
                v.get("name").and_then(|n| n.as_str()) == Some("my-installed-plugin")
                    && v.get("active").and_then(|a| a.as_bool()) == Some(false)
            });
            let Some(inactive) = inactive else {
                panic!("expected inactive my-installed-plugin event, got: {plugin_events:?}");
            };
            let reason = inactive
                .get("reason")
                .and_then(|r| r.as_str())
                .unwrap_or_default();
            assert!(
                reason.contains("crate(serde)"),
                "expected failing predicate reason to mention serde, got: {reason:?}"
            );
