...
Work on an open-source project or a team where people use different agents? No problem. Your Symposium configuration is agent agnostic, and `cargo agents` adapts it to the agent that each person is using. You can also specify the agent centrally if you prefer.
