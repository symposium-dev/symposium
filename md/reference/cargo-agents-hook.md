...
1. **Auto-sync** (if enabled) — when `auto-sync = true` in the user config, runs [`symposium sync`](./cargo-agents-sync.md) to ensure skills are current for the workspace. Failures are logged but don't block hook dispatch.
