...
1. **Auto-sync** (if enabled) — when `auto-sync = true` in the user config, runs [`symposium sync`](./sync-agent-flow.md) to ensure skills are current. The workspace root is resolved from the payload's `cwd` field. Runs quietly and non-fatally — failures are logged but don't block hook dispatch.
