...
2. **Dispatch to plugin hooks** — for each enabled plugin that defines a hook handler for the incoming event:
   - Ensure any declared `requirements` for the hook are installed (on-demand installation).
   - If the hook declares a `distribution`, resolve and execute the distributable; otherwise run the local `command` if present.
