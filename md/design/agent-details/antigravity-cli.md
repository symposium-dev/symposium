...
binary's hook proto, and it works: registered in `hooks.json` it loads, fires
once per session, and carries a populated `workspacePaths`. `PreInvocation` by
contrast fires before every model call, and its `invocationNum` restarts at 0
each turn, so it marks the start of a turn rather than of a session — which is
why symposium maps it to `user-prompt-submit` and gates it on `invocationNum == 0`.

Unknown event keys are accepted silently and never fire, so a misspelled event
name fails with no error at all.
