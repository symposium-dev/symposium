...
2. For each id the fixed-point calls `pms.load_plugin(id)` on the **package-manager set** `active_plugins` was handed (built once by `package_managers(deps)`). The id's `pm` routes it to the cargo transport, which:
   ...
   - `CargoPm::plugin_from_fetched` returns the crate as an unvalidated plugin, with a manifest `plugins::merge_crate_manifest` builds by layering two sources (merge order: `[package.metadata.symposium]` from `Cargo.toml` → `SYMPOSIUM.toml` file). Both use the ordinary plugin-manifest schema and are parsed **leniently** (a malformed layer is logged and dropped).
   - The cargo instance validates it (`plugins::validate_plugin` with `PluginKind::Package`), under `ManifestOrigin::Crate` (name defaults to the crate, `depends-on` is waived, `[defaults]` accepted, default `skills/` group appended unless `[defaults] skills = false`). The result is a `Plugin` whose `canonical` id is the resolved crate. A crate with no manifest sources still yields one whose only content is that default `skills/` group.
...
The key code paths are in `pm/cargo/mod.rs` (`CargoPm::load_plugin`, `plugin_from_fetched`), `pm/mod.rs` (`PmInstance`, `PluginKind`), `plugins.rs` (`merge_crate_manifest`, `validate_plugin`, `RawPluginManifest::merge`, `ManifestOrigin::Crate`, `Plugin::canonical`), `skills.rs` (`active_plugins`, `record_active`, `plugin_key`, `collect_skills`, `hash_origin_key`), `crate_metadata.rs` (`symposium_metadata`), `pm/cargo/workspace.rs` (`WorkspaceDeps`, `WorkspaceCrate`), and `crate_sources/mod.rs` (`RustCrateFetch`).
...
3. Independently, a registry plugin with no dependency gate anywhere loads *dormant* (`PluginManifest::requires_use`) and activates only when a `use` entry names it. The gate rides the `PredicateContext` (`with_used_names` / `is_used`), so skill resolution, hook dispatch, subcommand lookup, help, and MCP filtering all agree.
