...
1. **Determine agent** — check `.cargo-agents/config.toml` for a project-level `[agent]` override. If not set, fall back to the user-wide config.
...
3. **Install extensions** — read `.cargo-agents/config.toml` and, for each enabled extension:
...
1. **Read user config** — load `~/.cargo-agents/config.toml` to determine the agent.
