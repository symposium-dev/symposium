...
`resolve_applicable` is the entry point: it returns the applicable skills (each `SkillWithGroupContext` carries its owning plugin and the union of the witness crates across the plugin, group, and skill levels) plus a `PluginActivation` per matched plugin (needed because a plugin can match yet vend no skill). Witnesses are captured during the single gating pass via `PredicateSet::witness` rather than re-evaluated, so `shell` predicates are not run twice. `skills_applicable_to` is a test-only wrapper returning just the skills.
