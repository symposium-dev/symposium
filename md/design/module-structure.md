...
Implements `cargo agents use` and `cargo agents remove`. Parses `<CRATE>[@<VERSION>]`, `--git`, and `--path` forms, rejects mixed source forms, and mutates `[used.crates]`, `used.git`, or `used.paths` deterministically. Sync consumes these entries through the resolved source graph.
