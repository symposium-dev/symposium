...
Defines the user-wide `Config` (stored at `~/.symposium/config.toml`) with `[[agent]]` entries, logging, plugin sources (`[[plugins]]` entries with `PluginsEntry`/`PluginsEntrySource`), `[discovery.allow]` / `[discovery.deny]` policy, and `auto-update` (off/warn/on, default on). Each `[[plugins]]` entry carries optional `where.predicates` (for directory-scoping or other activation gates) and a `source` table with `crates`, `paths`, and `git` fields. A custom deserializer accepts both the legacy `[used]` / `[used.crates]` shape and the new `[[plugins]]` array, silently migrating the former on load. `plugins_to_used()` provides a compatibility view for code that still consumes the flat `UsedSources` structure. Legacy `[defaults]` and `[[plugin-source]]` config are rejected by the schema. The `workspace_deps(cwd)` factory is the standard way to create a `WorkspaceDeps` — it wires in `cargo_override` and `cache_dir` so callers get both the `SYMPOSIUM_CARGO` override and cross-invocation disk caching.
...
The registry entry point is `load_registry_from_graph(graph)`, which stamps each `ParsedPlugin` with the source node's provenance set so `workspace()`, `dependency()`, and `used()` predicates evaluate correctly.
...
Defines one `Predicate` enum covering both crate-graph matching and runtime/environment gating, plus `PredicateSet` (a list ANDed together) and `PredicateContext` (the workspace crate list and `cwd` it evaluates against). The `all_predicates_known(ctx)` helper on `PredicateSet` returns whether every predicate in the set can be evaluated given the current context (used by the deferred-source mechanism to detect entries gated on not-yet-defined custom predicates). Two surface syntaxes lower to the same tree:
...
- The **`predicates`** field uses function-call syntax: `crate(<atom>)`, `shell(<cmd>)` (verbatim arg, `sh -c`, exit 0 holds), `path_exists(<arg>)` (disk, then `$PATH` for bare names), `env(<name>[=<value>])`, `directory(<path>)` (exact match against `cwd`) / `directory(<path>/**)` (prefix match — holds when `cwd` is at or below `<path>`), the source-context predicates `workspace()`, `dependency()`, `used()` (evaluate against the non-exclusive provenance set on `PredicateContext`, updated per plugin before evaluation), and the combinators `not(<p>)`, `any(<p>, …)`, `all(<p>, …)`.
...
Source-context predicates (`workspace()`, `dependency()`, `used()`) evaluate against a `BTreeSet<SourceProvenance>` on `PredicateContext`. Each evaluation site (`skills_applicable_to`, `dispatched_hooks_for_payload`, `applicable_subcommands`, and the MCP collection loop in sync) sets the provenance from `ParsedPlugin::source_provenance` before evaluating that plugin's predicates. The provenance is non-exclusive — a source can simultaneously be `used`, `workspace`, and `dependency` — so all three predicates can be true for the same plugin. See the [predicates reference](../reference/predicates.md).
...
Also defines `ResolvedSourceGraph`, a pre-discovery graph that combines used sources, workspace sources, discovery-allowed dependency sources, and recursive plugin-source declarations. Nodes dedupe by canonical source path and keep non-exclusive provenance flags (`used`, `workspace`, `dependency`) plus human-readable reasons. Sync uses `load_registry_from_graph` to build the plugin registry from resolved source nodes, stamping each `ParsedPlugin` with the node's provenance. Resolution of crate-registry sources in use is best-effort: failures are logged and skipped rather than aborting sync. `expand_source_graph` uses a fixed-point worklist: it collects discovery policy from already-resolved plugins, evaluates workspace dependency candidates against the policy, and resolves recursive `[[plugins]] source.git`/`source.crate` declarations until the graph converges. Entries whose `where.predicates` reference custom predicates not yet defined are wrapped in `DeferredSource` and held back; when a later iteration resolves a plugin that defines the missing custom predicate, the deferred entry is retried. The worklist terminates when no new nodes are added and no deferred entries become unblocked. `entry_source_specs()` converts a `PluginsEntry` into the concrete `RegistrySourceSpec` values consumed by the resolver. Recursive source edges inherit the declaring source's full provenance set.

### `discovery.rs` — discovery policy evaluation

Implements the allow/deny specificity rules for workspace dependency auto-discovery. `CollectedPolicy` aggregates discovery rules from user config and plugin manifests in use. `DiscoveryCandidate` represents a candidate source (currently crate-registry candidates from workspace dependencies). The policy evaluator applies specificity-based matching: a specific rule beats a wildcard; if allow and deny have the same specificity, deny wins; and the default (no matching rule) is deny. Helper functions build candidates from workspace crates and resolve allowed candidates into source specs.
